...
def create_database_and_tables(cursor):
    """Create db and 3 tables: books, authors, genres"""
    try:
    ...
    try:
        ...
        with open("data/books.csv", "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                cursor.execute("insert into books (title, author_id, genre_id) values (%s, %s, %s)", (title, name_id, genre_id))
...
def main():
    try:
        ...
        if not conn:
            print("Connection unsucced (withoud db)")
            return
        ...
        if not conn:
            print("Connection unsucced (with db)")
            return
        cursor = conn.cursor()
