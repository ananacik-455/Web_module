...
def process_urls(url_list):
    ...
    for url in url_list:
        try:
            response = mock_requests.get(url)
        except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
            print(e)
        except requests.exceptions.HTTPError as e:
            print(f"Status code: {e.response.status_code}, URL: {e.response.url}")
        else:
...
urls_to_process = [
    ...
    "http://error.com/page2",    # Імітація ConnectionError
    "http://valid.com/page3",
    "http://timeout.com/page4",    # Імітація Timeout
    "http://valid.com/page5",
    "http://404.com/page6"    # Імітація HTTPError (404)
    ]
...
# mock_requests = MockRequests()
# response = mock_requests.get(urls_to_process[0])
# print(response)
